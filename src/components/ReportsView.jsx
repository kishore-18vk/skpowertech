...
import { downloadSalesPDF } from '../utils/pdfGenerator';
    ...
    <div className="space-y-6 animate-fade-in text-left">
      <div className="no-print-invoice space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-900 p-4 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm no-print">
          ...
          <div className="flex items-center space-x-2.5">
            <button
              onClick={() => downloadSalesPDF(monthlySales, selectedMonth === 'ALL' ? 'All Time' : getMonthYearString(`${selectedMonth}-01`))}
              className="flex items-center space-x-1.5 px-3.5 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-xl text-xs font-extrabold shadow-md shadow-emerald-500/20 transition-all duration-150"
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm print-card">
          ...
          {/* Main Aggregation Cards Grid (Hidden in PDF/Print) */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 my-8 no-print">
                      ...
                      <tr key={sale.id} className="text-slate-600 dark:text-slate-350 hover:bg-slate-50/50 dark:hover:bg-slate-800/10">
                        ...
                        <td className="py-3">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                            <span className="text-[10px] text-slate-400">{sale.category}</span>
                            {sale.serialNumber && (
                              <span className="text-[9px] font-extrabold text-amber-700 dark:text-amber-300 bg-amber-100/70 dark:bg-amber-950/50 px-1.5 py-0.5 rounded border border-amber-200 dark:border-amber-900">
